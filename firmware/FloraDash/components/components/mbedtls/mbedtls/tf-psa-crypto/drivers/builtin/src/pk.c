                                           ...
                                           mbedtls_svc_key_id_t *new_key_id)
{
    ...
    unsigned char *key_buffer = mbedtls_calloc(1, PSA_EXPORT_KEY_PAIR_MAX_SIZE);
    if (key_buffer == NULL) {
        return PSA_ERROR_INSUFFICIENT_MEMORY;
    }
    size_t key_length = 0;
    psa_status_t status = psa_export_key(old_key_id,
                                         key_buffer, PSA_EXPORT_KEY_PAIR_MAX_SIZE,
                                         &key_length);
    if (status != PSA_SUCCESS) {
                                  ...
                                  mbedtls_svc_key_id_t *key_id)
{
    ...
    switch (mbedtls_pk_get_type(pk)) {
        ...
        case MBEDTLS_PK_OPAQUE:
        {
            ...
            unsigned char *key_buffer = mbedtls_calloc(1, PSA_EXPORT_PUBLIC_KEY_MAX_SIZE);
            if (key_buffer == NULL) {
                return MBEDTLS_ERR_PK_ALLOC_FAILED;
            }
            status = psa_export_public_key(pk->priv_id,
                                           key_buffer, PSA_EXPORT_PUBLIC_KEY_MAX_SIZE,
                                           &key_length);
            if (status != PSA_SUCCESS) {
                mbedtls_free(key_buffer);
                return PSA_PK_TO_MBEDTLS_ERR(status);
    ...
    int ret = PSA_PK_TO_MBEDTLS_ERR(psa_import_key(attributes,
                                                key_data, key_length,
                                                key_id));
                        ...
                        unsigned char *sig, size_t sig_size, size_t *sig_len)
{
    ...
    if (ctx->pk_info->type == MBEDTLS_PK_RSASSA_PSS) {
        // mbedtls_rsa_alt_context *rsa_alt = ctx->pk_ctx;
        mbedtls_rsa_context *rsa = ctx->pk_ctx;
        if (rsa != NULL) {
            mbedtls_rsa_set_padding(rsa, MBEDTLS_PK_RSA_PKCS_V21, md_alg);
            return mbedtls_pk_sign(ctx, md_alg, hash, hash_len,
                                sig, sig_size, sig_len);
        }
    }
